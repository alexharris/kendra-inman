...
export default function Header() {
  ...
  // Function to get the average background color of the top 200px
  const getBackgroundColor = () => {
    try {
      ...
      // Sample multiple points across the top 200px area
      const samplePoints = [];
      ...
      for (let x = 0; x < width; x += step) {
        for (let y = 0; y < height; y += step) {
          ...
          if (element) {
            ...
            // Convert rgba/rgb to values we can work with
      ...
      // If we have samples, analyze the most common color
      if (samplePoints.length > 0) {
        // For simplicity, use the first non-transparent color found
  ...
  // Function to update fill color based on background
  const updateFillColor = () => {
    const bgColor = getBackgroundColor();
    const isLight = isLightColor(bgColor);
    setFillColor(isLight ? 'black' : '#F5F5DC'); // beige color for dark backgrounds
  };

  // Effect for initial color detection
  useEffect(() => {
    // Wait for page to fully load before detecting colors
    const detectOnLoad = () => {
      setTimeout(updateFillColor, 100); // Small delay to ensure elements are rendered
    };
    ...
    if (document.readyState === 'complete') {
      ...
      return () => window.removeEventListener('load', detectOnLoad);
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      <svg className="w-48 transition-colors duration-300" style={{ fill: fillColor }} viewBox="0 0 1330 124" fill="none" xmlns="http://www.w3.org/2000/svg">
