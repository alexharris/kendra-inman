...
export default function Header() {
  ...
  // Listen for animation completion event from homepage
  useEffect(() => {
    const handleAnimationComplete = () => {
      setLogoVisible(true);
    };

    window.addEventListener('headerLogoFadeIn', handleAnimationComplete);
    
    // If not on homepage, show logo immediately
    if (!isHomepage) {
      setLogoVisible(true);
    }

    return () => window.removeEventListener('headerLogoFadeIn', handleAnimationComplete);
  }, [isHomepage]);
  ...
  return (
    <div
      ref={headerRef}
      id="site-header"
      className="mheader flex justify-center items-center px-4 fixed w-full bg-transparent z-40 transition-transform duration-300 ease-in-out text-beige fill-beige translate-y-0"
    >
      <a href="/" className="futura-bold text">
      <svg className={`w-48 transition-all fill-inherit duration-500 ${logoVisible ? 'opacity-100' : 'opacity-0'}`} viewBox="0 0 1330 124" xmlns="http://www.w3.org/2000/svg">
